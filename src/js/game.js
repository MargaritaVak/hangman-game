...
const stopGame = (status) =>{
    ...
    if(status === 'win'){
        ...
        document.getElementById('game').innerHTML +='<h2 class="text-2xl font-bold mt-4 text-lime-600">You won!</h2>'
    } else if(status === 'lose'){
        document.getElementById('game').innerHTML +='<h2 class="text-2xl font-bold mt-4 text-red-600">You lost :( </h2>'
    } else if(status === 'quit'){
        logoH1.classList.remove('logo-sm')
        document.getElementById('hangman-img').remove();
    }

    document.getElementById('game').innerHTML += `<p class="text-xl">The word was: <span class="text-2xl font-medium uppercase">${word}</span></p><button id="play-again" class="button-primary px-5 py-3 mt-5">Play again</button>`;
...
export const startGame = () => {
  ...
  gameDiv.innerHTML +=
    `<p id="tries" class="mt-2">TRIES LEFT: <span id="tries-left" class="font-medium text-red-600">${triesLeft}</span></p>`;
